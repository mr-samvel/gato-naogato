...
def gather_dataset(dataset_path, dataset_type):
    with File(dataset_path, 'r') as dataset_file:
        dataset_x = numpy.array(dataset_file[f'{dataset_type}_set_x'][:]) # array de imagens de shape (num_imgs, width, height, camadas RGB (3))
        dataset_y = numpy.array(dataset_file[f'{dataset_type}_set_y'][:]) # array de labels p/ as imagens (0: nao gato; 1: gato)
